...
- **Note:** Stock prices are in each company's local trading currency and are not cross-currency comparable in raw form, use `Close_Norm_Mean` for comparisons.
- **Note:** Financials are similarly in local currency, use `Revenue_Growth_Pct` for cross-company comparisons.
